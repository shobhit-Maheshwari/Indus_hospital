import uuid
from typing import Any, AsyncGenerator, Union, TYPE_CHECKING
...
async def get_response(
    ...
    mongodb_saver: AsyncMongoDBSaver,
    ...
    Args:
        ...
        philosopher_era: Historical era of the philosopher (used for era guardrail).
    ...
    try:
        ...
        # Guard: summary must be a str (Redis may return None on cache miss)
        ...
        formatted_input = __format_messages(messages=messages)
        human_msg = formatted_input[-1].content if formatted_input else ""
...
async def get_streaming_response(
    ...
    try:
        ...
        # After streaming, we don't have the final state easily accessible here without running the graph
        # Normally streaming also tracks the full message. For write-behind, we should hook into the final state
        # In this implementation, the `generate_response.py` stream only yields chunks. 
        # The Orchestrator will need the full response. 
        # The caller (api.py) builds the full_response, so we let api.py call save_interaction for streams!
