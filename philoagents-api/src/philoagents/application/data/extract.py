...
def extract_wikipedia(philosopher: Philosopher) -> list[Document]:
    ...
    try:
        import wikipedia
        # Set custom User-Agent to bypass Wikipedia API rate-limiting/blocks
        wikipedia.set_user_agent("PhiloAgentsCourse/1.0 (contact@example.com)")
    except Exception as e:
        logger.warning(f"Could not set custom Wikipedia User-Agent: {e}")
    ...
    try:
        ...
        logger.warning(f"Failed to extract Wikipedia page for {philosopher.name}: {e}")
        return []
