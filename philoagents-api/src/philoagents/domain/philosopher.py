...
class Philosopher(BaseModel):
    ...
    Args:
        ...
        era (str): The historical era the philosopher lived in. Used as a
            guardrail so the agent does not answer questions about events
            or concepts that postdate their lifetime.
    """
    ...
    era: str = Field(
        description="Historical era of the philosopher, used as a temporal guardrail"
    )

    def __str__(self) -> str:
        return (
            f"Philosopher(id={self.id}, name={self.name}, "
            f"perspective={self.perspective}, style={self.style}, era={self.era})"
        )
